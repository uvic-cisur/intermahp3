## code to prepare `ihme_high` dataset goes here
...
ihme_high_rr = left_join(ihme_rr2, ihme_causes, by = c('cause_name' = 'condition')) %>%
  select(im, gender, outcome, risk) %>%
  mutate(bingea = 0, wholly_attr = FALSE, r_fd = 0, binge_risk = NA)

usethis::use_data(ihme_high_rr, overwrite = T)
